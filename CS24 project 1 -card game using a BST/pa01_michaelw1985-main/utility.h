...
struct Node {
    ...
    // useful constructor:
    Node(card v = {"none", "none"}) : info(v), left(0), right(0), parent(0) { }
    void print() {
        cout << info.getSuit() << " " << info.getValue() << endl;
    }
...
bool checkInOrder(Node* n, cardBST& other);
bool checkRevInOrder(Node* n, cardBST& other);

public:
//0-case card for failed if-branches
card failCard() const;

//in-order and reverse in-order checking
bool checkInOrder(cardBST& other);
bool checkRevInOrder(cardBST& other);
